 ...
 * delegated by imitation. Exactly one of the two instructions below is always
 ...
 * turn's cost to the background text plus a sentence.
 */
...
export function composeStepInput(
	...
	const delegated = step.useSubagent;
	...
	return `${preamble}${tcpBlock}${resourcesBlock}${step.description}${descriptionImages}${criteriaNote(
		...
		acceptanceImages,
	)}${priorResults}${subagentInstruction(step.useSubagent)}${
...
export async function dispatchStep(
	...
	const sessionToResume =
		mode === "judge"
			...
			: (options.resumeSession ?? true)
				...
				: null;
	// URLs handed to the agent inside a docker sandbox must name an address the
	// container can reach (sandbox-net.ts). The broker's `started`/`result`
	// callbacks are different: awb posts them from the host and only accepts
	// loopback targets (`127.0.0.1` / `localhost` / `[::1]`), so those stay on
	// loopback even when the step runs in a container.
	const sandboxed = hookRuntime(workflow.hookUrl).sandbox != null;
	const agentHubHost = hubHostForStep(cfg, sandboxed);
	const agentHubOrigin = `http://${agentHubHost}:${cfg.port}`;
	const brokerHubOrigin = `http://127.0.0.1:${cfg.port}`;
	...
	if (sandboxed && !hubReachableFromSandbox(cfg)) {
		log(
			`step ${step.id} (workflow ${workflow.id}) runs in a docker sandbox, but the hub is bound to ${cfg.host} — ` +
				`a container cannot reach that for its TCP tools. ` +
				`Set "host" in ~/.target/config.json to an address the bridge can reach (0.0.0.0, or ${agentHubHost}) and restart the hub.`,
			"warning",
		);
	}
	const callbackUrl = `${brokerHubOrigin}/api/steps/${step.id}/result?token=${step.callbackToken}`;
	...
	const startedCallbackUrl = `${brokerHubOrigin}/api/steps/${step.id}/started?token=${step.callbackToken}`;
	...
	const tcpExecuteUrl = `${agentHubOrigin}/api/tcps/execute?stepId=${step.id}&token=${step.callbackToken}`;
