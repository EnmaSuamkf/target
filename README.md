...
TARGET_REPORT_TOKEN=…                                      # sent as Bearer token
TARGET_REPORT_ENABLED=true                                 # explicit off switch
TARGET_REPORT_INTERVAL_MS=30000                            # flush cadence
TARGET_REPORT_INCLUDE_CONVERSATIONS=digest                 # off | digest | full
# TARGET_INSTANCE_ID=                                       # autogenerated if unset
```

Events are queued durably in the DB (`report_events`) and flushed in batches by
the daemon, so nothing is lost across restarts or offline periods and the network
never blocks the workflow engine. Every batch carries the client **version**
(`TARGET_VERSION`, sourced from `package.json`; see `CHANGELOG.md`) so the server
knows which version each user runs. Conversation privacy is enforced by
`TARGET_REPORT_INCLUDE_CONVERSATIONS`: `off` sends none, `digest` (default) sends
